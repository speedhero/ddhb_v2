...
public class MemberRegisterAction {
	...
	private static final Logger LOGGER = Logger.getLogger(MemberRegisterAction.class);
	 ...
	 * @return
	 */
	@RequestMapping(params = { "actionMethod=register" }, method = { RequestMethod.GET })
	 ...
	 * @return
	 */
	@RequestMapping(params = { "actionMethod=preRegist" }, method = { RequestMethod.GET })
	public String checkRegister(HttpServletRequest request) {
		String referer = request.getHeader("referer");
		request.getSession().setAttribute("previousUrl", referer);
		return "redirect:/register.show?actionMethod=register";
	}

	@RequestMapping(params = { "actionMethod=register" }, method = { RequestMethod.POST })
	public String processRegister(Model model, @ModelAttribute final PlatMemberInfo platMemberInfo, BindingResult result,
		...
		if (result.hasErrors()) {
			...
			(new Thread() {
				public void run() {
					try {
						MemberRegisterAction.this.sendEmailService.sendEmail(emailMessage1, platMemberInfo.getEmailAddress());
					} catch (Exception var2) {
						MemberRegisterAction.LOGGER.error(var2.getMessage());
	...
	@RequestMapping(params = { "actionMethod=activity" })
	...
	private void validate(PlatMemberInfo platMemberInfo, Errors errors, HttpServletRequest request) {
		...
		if (StringUtil.isEmpty(platMemberInfo.getPassWd())) {
			errors.reject("请输入密码");
		} else if(platMemberInfo.getPassWd().toString().getBytes(Charset.forName("GBK")).length < 6){
...
//		}
		...
		String passWord = platMemberInfo.getPassWd();
	...
	@RequestMapping(params = { "actionMethod=activeAccount" })
		...
		if (!CommonUtil.isNotZeroLengthTrimString(uuId)) {
			...
			if (platMemberInfo != null) {
				...
				while (integrateRulesPhone.hasNext()) {
					IntegrateRule rulesPhone = (IntegrateRule) integrateRulesPhone.next();
					if (rulesPhone.getRuleName().equals("registed")) {
						...
						integralHistory.setIntegral(rulesPhone.getRuleValue());
						integralHistory.setGetflag(rulesPhone.getId().intValue());
						tIntegralTable.setIntegral(rulesPhone.getRuleValue());
					} else if (rulesPhone.getRuleName().equals("maxChangeLimit")) {
						tIntegralTable.setChangedCount(0);
					} else if (rulesPhone.getRuleName().equals("maxReceivedLimit")) {
						tIntegralTable.setReceivedCount(0);
					}
				}
				...
				if (platMemberInfo.getMobilephoneValidatedFlag() == 1) {
					...
					IntegralHistory integralHistoryPhone = new IntegralHistory();
					integralHistoryPhone.setPlatMemberInfo(platMemberInfo);
					Iterator var14 = integrateRulesPhone1.iterator();

					while (var14.hasNext()) {
						IntegrateRule integrateRule = (IntegrateRule) var14.next();
						if (integrateRule.getRuleName().equals("phoneValidated")) {
							...
							integralHistoryPhone.setIntegral(integrateRule.getRuleValue());
							integralHistoryPhone.setGetflag(integrateRule.getId().intValue());
							tIntegralTable.setIntegral(tIntegralTable.getIntegral() + integrateRule.getRuleValue());
						}
					}

					this.platMemberInfoService.save(integralHistoryPhone);
				}
	...
	@RequestMapping(params = { "actionMethod=forgetPassword" }, method = { RequestMethod.GET })
	...
	@RequestMapping(params = { "actionMethod=forgetPassword" }, method = { RequestMethod.POST })
	public String doForgetPassword(Model model, @ModelAttribute PlatMemberInfo platMemberInfo, BindingResult result,
		...
		if (result.hasErrors()) {
			...
			(new Thread() {
				public void run() {
					try {
						MemberRegisterAction.this.sendEmailService.sendForgetPasswordEmail(emailMessage,
	...
	@RequestMapping(params = { "actionMethod=modifyPassword" }, method = { RequestMethod.GET })
	...
	@RequestMapping(params = { "actionMethod=modifyPassword" }, method = { RequestMethod.POST })
	public String doChangePassword(Model model, @ModelAttribute PlatMemberInfo platMemberInfo, BindingResult result,
		...
		String oldPassword = platMemberInfoSession.getPassWd();
	...
	@RequestMapping(params = { "actionMethod=findPassword" }, method = { RequestMethod.GET })
	...
	@RequestMapping(params = { "actionMethod=findPassword" }, method = { RequestMethod.POST })
	...
	@RequestMapping(params = { "actionMethod=getPhoneMessage" }, method = { RequestMethod.POST })
	...
	private String createXml(String sessionId, String phoneNumber, PlatMemberInfo member, String message) {
		...
		if (member == null) {
			...
			if (member.getId() == null) {
				...
				reqXml.append("<MemberID>").append(member.getId()).append("</MemberID>");
			}
