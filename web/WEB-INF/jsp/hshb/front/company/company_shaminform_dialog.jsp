...
<form:form modelAttribute="shamInfo" id="shamInfo" action="${globalUrl}company/subservice">
<div class="tcc_zpShamInfo" >
	<div class="yyfw_con">
		...
		<div class="yyfw_bg">
		<c:if test="${houseNew ne null }">
			<strong>举报房源：<c:out value="${houseNew.buildingName}"></c:out></strong>
		</c:if>
		<c:if test="${houseNew eq  null }">
		<strong>举报房源：<c:out value="${house.community.communityName }"></c:out></strong>
		<span><fmt:formatNumber pattern="#" value="${house.area }"/>  平米</span><span>${house.shi }室${house.ting }厅${house.wei }卫</span><span><c:if test="${houseType == 1}"><b><fmt:formatNumber value="${house.price/10000 }"/></b>万元</c:if><c:if test="${houseType == 2}"><b><fmt:formatNumber value="${house.rentPrice }"/></b>元/月</c:if></span>
		</c:if>
		</div>
		<div class="yyfw_ls">
			<ul>
                <li style="width:100%;"><span>姓名：</span>
                	<input id="username" type="text" name="name" value="${LoginMember.accName}" style="width:64%; height:24px;">
                </li>
                <li style="width:100%;"><span>邮件：</span>
                    <input id="emailAddress" type="text" name="emailAddress" value="${LoginMember.emailAddress}" style="width:64%; height:24px;">
                </li>
