...
<div class="about_main"> 
	...
	<!--右侧开始-->
        <div class="cp_rt">
        	<div class="licn"><a href="${globalUrl}"  style="cursor: pointer;">首页</a>&nbsp;>&nbsp;新闻动态_hshb</div>
        	<div class="cont_news">
			<c:forEach var="item1" items="${typeList}" varStatus="status">
        	<c:set var="countItem" value="0"></c:set>
            <div id="title${item1.typeId}" class="<c:if test="${status.index != 0 }">none_ip</c:if>">
            	<div  class="news_tt">
            	<span><a href="${globalUrl}company/showNewsTypeList/${item1.typeId}" >查看更多</a></span>
            	<b>
                ...
                <div class="news_ls">
                	<ul>
                		<c:forEach var="item2" items="${newsContentList}">
                		<c:if test="${countItem<5 }">
                		<c:if test="${item2.newsType == item1.typeId}">
                		<c:set var="countItem" value="${countItem+1}"></c:set>
                    	<li><span><fmt:formatDate value="${item2.publishTime}" pattern="yyyy-MM-dd" /></span>
                    	<a href="${globalUrl}company/showNewsDetail/${item2.newsId}">
                    	<c:choose>  
                ...
                <div class="mob_mor"><a href="${globalUrl}company/showNewsTypeList/${item1.typeId}" >更多</a></div>
            </div>
...
function showMore(typeId){
	window.location.href = '${globalUrl}company/showNewsTypeList/' + typeId;
}
