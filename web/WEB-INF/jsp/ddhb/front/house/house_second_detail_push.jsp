...
function initCharts(size){
	var arrayz = [
	          	<c:forEach var="trend" items="${trendList}" varStatus="s">
	          	    ["${trend.calculateDate}".substring(0,10), ${trend.priceAnnounced}]${s.last?'':','}
	          	</c:forEach>
	          	]
	            ...
	            if(pois.length == 0){
	            	...
	            	for(var i = 0; i < pois.length; i++){
	            		...
	            		$('<div style="width: 100%; margin-top:5px;">'+
		            			'<div style="width:63%; float: left; border-bottom:1px dashed #F5F5F5; border-right: 1px solid #F5F5F5;">'+'<div style="background-image:url(images/housedetail/distantCircle.png) ;float:left;font-size:10px;color:#ffffff;width:14px;height:14px;line-height:13px;">'+(i+1)+'</div>'+
								...
								'<div style="width:36%;float: left; text-align: center; border-bottom:1px dashed #F5F5F5;">'+
									'<span class="mapFontSize" style="color:#575757">距离<span class="mapWordFontSize" style="color:#3fb8b1;">'+pois[i].dist+'</span>米，步行约<span class="mapWordFontSize" style="color:#3fb8b1;">'+min+'</span>分钟'+'</span>'+
								'</div>'+
...
//计算器
function openDialog(){
	var actionUrl = "${globalUrl}houseSecond.show?actionMethod=getCalculator&unitPrice=${house.unitPrice}&area=${house.area}";
	...
	_openDialog('calculator', "&nbsp;&nbsp;个人住房商业贷款", actionUrl, "get", "html", gray, dlzc, css);
}
//初始化二维码
function initQRCode(globalUrl){
	var url = "${globalUrl}houseSpecial.show?actionMethod=drawcode&houseId=${houseId}&agentId=${houseId}&houseType=1&houseNo=${house.houseNo}&pictureType=";
	_initQRCode(url);
}
...
<div class="zhxq_t">
        	<div class="zh_dbt">${house.title}</div>
            <div class="dt_pt">
                ...
                <c:forEach items="${tagList}" var="tags">
								<c:forEach items="${house.tagIdList}" var="tagId">
									<c:if test="${tags.erpId == tagId}">
										<span style="background-color:${tags.tagColor};color:${tags.fontColor};">
											${tags.tagName}
										</span>
									</c:if>
								</c:forEach>
            ...
            <div class="jia_pm">
            	<div class="jpm_s">
                	<div class="sjia"><p>售价（万元）</p><b><fmt:formatNumber value="${house.price/10000}"/></b></div>
                    <div class="mianji"><span>面积：</span><b><fmt:formatNumber pattern="#" value="${house.area}"/></b>（平米）</div>
                    <div class="danjia"><p>单价（元/平米）</p><b><fmt:formatNumber pattern="#" value="${house.unitPrice}"/></b></div>
                </div>
                <div class="jpm_x">
                	<div class="sjiang">
                    ...
                    <div class="daik"><span>贷款助手：</span><fmt:formatNumber type="number" value="${loanMonths / 12}"/>年   ${loanRatios}成商贷  <b><fmt:formatNumber value="${loanAssets }" pattern="##" ></fmt:formatNumber></b>元/月<a href="javascript:void(0);" onclick="openDialog();">房贷计算器</a></div>
                </div>
            </div>
            <div class="xs_k">
            	<div class="xs">
                ...
                <div class="san_k">
                	<ul>
                    	...
                    	<li><p>浏览量</p><b>1</b></li>
                    	<li class="nob"><p>评价</p><b>${appraiseList.size()}</b></li>
                    </ul>
                </div>
            ...
            <div class="man">
            	<div class="r_bh">内部编号：<br>${house.houseNo}</div>
                <div class="wd_lx">需要帮助请联系我</div>
                <div class="rwtp"><a href="#"><img src="${pictureHost}${house.broker.photograph}" onclick="window.open('${globalUrl}broker.show?actionMethod=brokerDetail&brokerId=${house.broker.erpId}&housetype=1')" title="查看${house.broker.bname}的资料和全部房源" alt=""/></a></div>
                <div class="m_con">
                    <div class="r_bh_c">内部编号：<br>${house.houseNo}</div>
                    ...
                    <div class="qqwx"><a href="tencent://message/?uin=${house.broker.qq }&Site=qq&Menu=yes" onclick="qqcao(${house.broker.qq })"><img src="${globalUrl}image/qq.gif" alt=""/></a><a ><img src="image/wx.gif" onclick="wchatDisplay();"/></a></div>
                </div>
                <div class="ewm" style="overflow: hidden;"><img id="qrCode" style="float:right;"  width="68" height="67" alt=""/><p><a onclick="alertQrCode2();">扫描二维码获取房源信息</a></p></div>
            </div>
        </div>
        <!-- 二维码放大隐藏 -->
    	...
    	<!--顾问开始-->
        <div class="guwen">
        	<div class="nr_tt"><span>共有 ${house.evaluationList.size()} 位置业人员服务于本房源</span><b>服务本房源的置业顾问：</b></div>
            <div class="gw_ls">
            	<ul>
            	<c:forEach items="${house.evaluationList }" var="apps" begin="0" end="2">
                	<li>
                        ...
                        <div class="gw_r">
                            ...
                            <c:if test="${apps.broker.weCharUrl != '' && apps.broker.weCharUrl != null}"><span><img src="${globalUrl}image/wx.gif" onclick="_showWeixin('${apps.broker.erpId}');" alt=""/></span></c:if></div>
