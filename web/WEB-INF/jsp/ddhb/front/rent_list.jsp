		...
		<c:forEach var="house" items="${pageBean.dataList}" varStatus="item">
			<li style="height:353px;">
				<div class="pt" onclick="window.open('${globalUrl}rent.show?actionMethod=houseRentDetail&houseNo=${house.hourseNo}&brokerId=${(houseAppraise != null) ? houseAppraise.broker.erpId : house.broker.erpId}')" style="cursor: pointer;">
					 ...
					 <div class="inf_div">
					    <p class="fl">共${house.housePicSize}张</p>
						<p class="fr">[${house.rentType.rentTypeName}]</p>
					 </div>
				</div>
				...
				<div class="tj_txt" style="height:168px;">
					...
					<div class="chengxq">${house.community.communityName }&nbsp;&nbsp;
						${house.community.region.countyName } - ${house.community.cbd.parentCBD.name }</div>
					<div class="tj_sx">					
						<span><fmt:formatNumber pattern="#">${house.area}</fmt:formatNumber>平米&nbsp;</span>
						<span>${house.shi}室${house.ting}厅${house.wei}卫</span>
						<!-- 添加对比 收藏  start-->
							<a housePictureUrl="${pictureHost}${house.pictureUrl }" inCompareItem="false" class="dbi_lik fr" onclick="event.cancelBubble = true;addCompareItem(this, 'rentHouseCompare')"
							 ...
							 <c:if test="${LoginMember != null }">
								<c:if test="${house.collectId != null}">
									<a id="collect${house.erpId}two"  href="javascript:void(0);" brokerid="${house.broker.erpId }" class="shouc_lik fr" collId="${house.collectId}" hosueobjectId="${house.erpId}" houseprice="${house.rentPrice}" isCollect="0"
