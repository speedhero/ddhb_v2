...
/**上方小地图**/
function initMapOnTheLeft(){
	var arr = "${community.location}";
...
//初始化街景地图和小区配套的地图
function initJJAndMap(){
	//点击小区配套按钮
	$("#checkbtnFacility").click(function(){
		$("#checkbtnFacility").addClass("one");
		$("#checkbtnJJ").removeClass("one");
		$("#streetmap").css("display", "none");
		$("#streetstreet").css("display", "block");
	});
	//点击小区街景按钮
	$("#checkbtnJJ").click(function(){
		$("#checkbtnJJ").addClass("one");
		$("#checkbtnFacility").removeClass("one");
		$("#streetmap").css("display", "block");
		$("#streetstreet").css("display", "none");
	});
  ...
  var arr = "${community.location}";
	...
	var icon1 = new soso.maps.MarkerImage('images/maker.png', size1, anchor1);
		...
		complete : function(results){
			...
			if(pois.length == 0){
				...
				for(var i = 0; i < pois.length; i++){
					...
					'<span style="float: left; margin-left:6px;font-size:12px;color:#666666">'+pois[i].name +"&nbsp;&nbsp;"+ pois[i].address+'</span>'+
					'</div>'+
					'<div style="width:36%;float: left; text-align: center; border-bottom:1px dashed #F5F5F5;">'+
					'<span style="font-size:12px;color:#575757">距离<span style="color:#3fb8b1;font-size:16px;">'+pois[i].dist+'</span>米，步行约<span style="color:#3fb8b1;font-size:16px;">'+min+'</span>分钟'+'</span>'+
					'</div>'+
	...
	_searchKeyword("交通",20,'traffic');
}

//周边设施
function _searchKeyword(keyword, number, divId){
	searchKeyword(keyword, number, divId, '${community.location}');
}
--%>
//小区地理坐标
...
<!--头部结束-->
<div class="Location">
	<a onclick='window.location.href="${globalUrl}welcome.show?actionMethod=welcome"'>首页</a> > 
	<a onclick='window.location.href="${globalUrl}community.show?actionMethod=dimquery&type=0"'>杭州小区</a> > 
	  ...
	  <div class="qian_k">
    	<div class="q_s">
            ...
            <div class="q_sx">
                ...
                <!--<p>同比去年：
                	<c:if test="${community.aroundLastYear eq 0}">
                		<span class="s">${community.aroundLastYear }%</span>
               		</c:if>
               		<c:if test="${community.aroundLastYear gt 0}">
               			<span class="s">${community.aroundLastYear }%</span>
              		</c:if>
              		<c:if test="${community.aroundLastYear lt 0}">
              			<span class="x"><c:out value="${fn:substring(community.aroundLastYear, 1, 5 )}"></c:out>%</span>
             		</c:if>
				</p>-->
            </div>
        </div>
        ...
        <div class="tit_a"  id="titleBar">
        	...
        	<div id="nvBar">
			    <div id="barbar">
	        		...
	        		<a id="houseSecondBar" onclick='changeSecondAndRent("1");' href="#communityRentSecond">二手房源</a>
	        		<a id="houseRentBar" onclick='changeSecondAndRent("2");' href="#communityRentSecond">出租房源</a>
	       		</div>
       		</div>
