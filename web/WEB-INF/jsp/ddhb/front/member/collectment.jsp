...
<div class="cp_rt">
        	<div class="licn"><a onclick='window.location.href="${globalUrl}welcome.show?actionMethod=welcome"'>首页</a>&nbsp;>&nbsp;个人中心&nbsp;>&nbsp;收藏夹管理</div>
                        ...
                        <ul>
                            ...
                            <li class="li_2">
								<c:if test="${house.collectType eq 0 }">
									<a href="${globalUrl}chushou/${house.houseSecond.houseNo}.html"><c:out value="${house.houseSecond.title }"></c:out></a>
								</c:if>
								<c:if test="${house.collectType eq 1 }">
									<a href="${globalUrl}chuzu/${house.houseRent.hourseNo}.html"><c:out value="${house.houseRent.title }"></c:out></a>
								</c:if>
								<c:if test="${house.collectType eq 2 }">
									<a href="${globalUrl}xiaoqu/${house.community.erpId}.html"><c:out value="${house.community.communityName }"></c:out></a>
								</c:if>
								<c:if test="${house.collectType eq 4 }">
									<a href="${globalUrl}xinfang/${house.houseNew.id}.html"><c:out value="${house.houseNew.buildingName }"></c:out></a>
								</c:if>
							</li>
                            ...
                            <li class="li_5"><span>价格/租金:</span>
								...
								<c:if test="${house.collectType eq 4 }">
									<fmt:formatNumber value="${house.houseNew.minPrice }" pattern="0"/>万元
								</c:if>
							</li>
