...
<div class="cp_rt">
     	<div class="licn"><a onclick='window.location.href="${globalUrl}welcome.show?actionMethod=welcome"'>首页</a>&nbsp;>&nbsp;个人中心&nbsp;>&nbsp;预约管理</div>
         ...
         <div class="gr_ls">
             ...
             <div class="gr_nlb">
             	<div class="gr_nrb">
					...
					<c:if test="${house.houseType eq 4 }"><c:out value="${house.houseNew.buildingName }"></c:out></c:if>
				</li>
                         <li class="li_3"><span>类型:</span>
					<c:if test="${house.houseType eq 1 }">二手房</c:if>
					<c:if test="${house.houseType eq 2 }">租赁房</c:if>
					<c:if test="${house.houseType eq 4 }">新房</c:if>
				</li>
				...
				<c:if test="${house.houseType eq 4 }">
