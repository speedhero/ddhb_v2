...
function toDetailPage(positionId){
	...
	window.open("${globalUrl}company.show?actionMethod=positionDetail&positionId=" + positionId);  
}
//判断设备的屏幕是不是小屏幕
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<c:forEach items="${pageBean.dataList }" var="position" >
		<tr onclick="toDetailPage('${position.id }')">
			<td>
				<a class="hasUnderLine" href="${globalUrl}company.show?actionMethod=positionDetail&positionId=${position.id }">
		...
		<td><a class="yp" style="padding:0; background:none;" href="${globalUrl}company.show?actionMethod=positionDetail&positionId=${position.id }&isDo=true">我要应聘</a></td>
	</tr>
	</c:forEach>
</table>
<div class="page" style="display:block;">
<huatek:ddhbajaxCutPage pageBean="${pageBean}" isOuterForm="false" formName="positionForm" offerPageSize="20,50,100" isExistForm="true" queryFunction="changePages()"/>
</div>
<!--右侧结束-->
<script>
function changePages() {
	...
	postDataByURL2('${globalUrl}company.show?actionMethod=cutPostype',jsoninit,"changelist");
}
</script>