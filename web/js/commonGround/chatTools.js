...
function _showWeixin(tagName,id,title){
//	var dataContent = document.getElementById("BWeixin_" + broker_id);
	var dataContent = document.getElementById(tagName);
	art.dialog({
		id:id,					//'BWeixin'
 		title: title, 			//"评价人微信"
 		content: dataContent,
...
//初始化二维码 租赁houseType = 2 ;二手房houseType = 1 
//url = "${globalUrl}houseSecond.show?actionMethod=drawcode&id=${houseAppraise.erpId}&pictureType=";
function _initQRCode(actionUrl){
  var url = actionUrl;
...
//  if(houseType)
//  	url += "&houseType="+ houseType +"&houseId=" + houseNo;
  $("#qrCode").attr("src", url);
}
