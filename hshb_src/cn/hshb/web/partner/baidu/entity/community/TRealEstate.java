...
public class TRealEstate extends BaseEntity {
    ...
    public static TRealEstate fromCommunity(Community comm){
...
//    	r.setGreenRate(BigInteger.valueOf(Long.valueOf(String.valueOf( comm.getLandScaping() * 100L ))));
    	r.setGreenRate(BigInteger.valueOf((long)( comm.getLandScaping() * 100L )));
    	r.setHouseAge(StringUtil.yearToStr(comm.getBuildYear()) + "年");
    	r.setHouseType(THouseType.普通住宅.value());
    	r.setIntroduce(strToCDATA(comm.getCommunityName(), ""));  //TODO:这里是小区介绍，暂时没有，用小区名称代，以后要改
...
//    	r.setPublishTime(StringUtil.dateToStr(Calendar.getInstance().getTime()));
    	r.setPublishTime(StringUtil.dateToStr(comm.getBaiDuCommuntiy().getPublishTime()));    	
    	r.setLastmod(StringUtil.dateToStr(comm.getBaiDuCommuntiy().getLastmod()));
    	r.setName(strToCDATA(comm.getCommunityName(), ""));
    	r.setOnrentNum(BigInteger.valueOf(comm.getHouseCount()));	//这里在售和在租的房源量，需要修改
    	r.setOnsaleNum(BigInteger.valueOf(comm.getHouseCount()));
    	r.setPlace(tPlace);
    	r.setPrice(BigInteger.valueOf(Float.valueOf(comm.getCurrentSHAvePrice()).longValue())); //买卖房源均价
    	...
    	// 周边设施
    	for(String cFacilities : comm.getCommunityFacilities())
