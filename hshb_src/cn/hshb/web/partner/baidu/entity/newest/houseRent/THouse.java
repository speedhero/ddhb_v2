...
public class THouse {
    ...
    public static THouse fromHouse(HouseRentHouseExt house){
    	if(house.getPictures()==null || house.getPictures().size()<1)
    	{
    		System.out.println("The other picture size <0	 rentHouse_id:" + house.getHouseId());
    		return null;
    	}
    	if(house.getCoverImage() == null ){
    		System.out.println("The cover image is empty rentHouse_id:" + house.getHouseId());
    		return null;
    		}
    	THouse h = new THouse();
    	h.setBuildNum("0");  //无楼号
    	...
    	//装修
    	if(StringUtil.isNotEmpty(house.getDecorationName()))
    		h.setDecoration(TDecoration.fromHBValue( house.getDecorationName()));
    	else
    	...
    	for(HousePicture hp : house.getPictures())
    		h.getHouseImage().add("http://"+StringUtil.HSHB_DOMAIN + "/frontImages" + hp.getPictureUri());
 
    	h.setHouseImageNum(String.valueOf(house.getPictures().size())+"张");
    	
    	String fn = "中";
    	if(house.getHouseVervicalLocation() == 1)
    		fn = "下";
    	else if(house.getHouseVervicalLocation() == 3)
    		fn = "上";
    	//总楼层数
    	h.setHouseFloor((house.getStoreyCount() == null?5:house.getStoreyCount())+"层");
    	//为空时，其他
    	...
    	if(house.getPropertyTypeName()!=null)
    	...
    	String url = "http://"+StringUtil.HSHB_DOMAIN+"/chuzu/"+house.getHouseId()+".html";
    	h.setUrl(url);
