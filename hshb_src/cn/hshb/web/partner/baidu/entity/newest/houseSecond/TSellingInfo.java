...
public class TSellingInfo extends BaseEntity{
	...
	public static TSellingInfo fromHouse(HouseSecondHandHouse house){
    	...
    	if(StringUtil.isEmpty(title)){
    				System.out.println("title is NULL secondHouse:" + house.getHouseId());
    				return null;
    	}
    	TSellingInfo sell = new TSellingInfo();
    	if(house.getPublisher()!=null){
	    	sell.setBroker(strToCDATA(house.getPublisher().getAgentName(), "未知"));
	    	sell.setBrokerTel(strToCDATA(house.getPublisher().getTelephone(), "18967113307"));
    	}
...
//    	sell.setPublishTime(StringUtil.dateToStr(Calendar.getInstance().getTime()));
    	sell.setLastmod(StringUtil.dateToStr(house.getBaiduHouseLastmod()));
    	sell.setPublishTime(StringUtil.dateToStr(house.getBaiduHousePublishTime()));
    	...
    	//sell.setSellingPriceAvg(StringUtil.moneyToStr(house.getPrice() / house.getArea())+"元/m2");
    		sell.setSellingPriceAvg(StringUtil.moneyToStr(house.getPrice() / house.getArea()));
    	//用于删除
...
//    	String tagsErpId = "b91edfdd-5d11-48c0-af89-211ec3ae53fc";
    	...
    	int tag = 64;
    	int houseTag = Integer.parseInt((String) (StringUtil.isEmpty(house.getTags())?"0":house.getTags()));
    	if((houseTag & tag) == tag)
    		sell.setIsFreeTax(1);
    	else
    		sell.setIsFreeTax(0);
