...
public class TContact extends BaseEntity {
    ...
    public static List<TContact> fromHouse(HouseSecond house){
...
//    	String publisherId = house.getBroker().getErpId();	//房源发布人ID
    	...
    	for(HouseAppraise app: appList){
    		...
    		if(app.getBroker()!=null){
	    		...
	    		c.setTelephone(strToCDATA(app.getBroker().getTelephone(), "18967113307"));
    		}else{
    			continue;
    		}
    		c.setType(T1Type.中介);
    	...
    	if(!isFoundPublisher){
    		...
    		if(house.getBroker()!=null){
    			...
    			System.out.println("No broker secondHouse: " + house.getHouseNo());
    			return null;
    		}
    		c.setType(T1Type.中介);
    		c.setWorksFor(StringUtil.HSHB_NAME);
