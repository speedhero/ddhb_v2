...
public class TRentalInfo extends BaseEntity {
    ...
    public static TRentalInfo fromHouse(HouseRent house){
    	...
    	if(contactList == null ||  contactList.size() < 1)
    		return null;
    	if(house.getFurnitureList() == null || house.getFurnitureList().size() < 1){
    		System.out.println("Indoor facilities	rentHouse_id:" + house.getHourseNo());
    		return null;
    	}
    	...
    	if("".equals(title.trim())){
    		System.out.println("title is null rentHouse_id:" + house.getHourseNo());
    		return null;
    	}
    	}
...
//    	rent.setPublishTime(StringUtil.dateToStr(Calendar.getInstance().getTime()));
    	rent.setLastmod(StringUtil.dateToStr(house.getBaiDuHouseRentList().getLastmod()));
    	rent.setPublishTime(StringUtil.dateToStr(house.getBaiDuHouseRentList().getPublishTime()));
    	...
    	rent.setRentalType(TRentalType.fromHBValue(house.getRentType().getRentTypeName()));
...
//    	rent.getContact().addAll(TContact.fromHouse(house));
    ...
    private List<TInstallations> getInstallations(HouseRent house){
    	List<TInstallations> list = new ArrayList<TInstallations>();
    	List<Furniture> furList = house.getFurnitureList();
//    	List<String> listStr = new ArrayList<String>();
    	for(Furniture fur: furList){
    		boolean b = true;
    		for(TInstallations t : list){
    			
    			if(t.value().equals((TInstallations.fromHBValue(fur.getFurName()).value())))
    			{
    				b = false;
    				break ;
    			}
...
//    	System.out.println("TInstallations.fromHBValue(fur.getFurName())" + TInstallations.fromHBValue(fur.getFurName()));
    	list.add(TInstallations.fromHBValue(fur.getFurName()));
