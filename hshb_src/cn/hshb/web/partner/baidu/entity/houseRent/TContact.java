...
public class TContact extends BaseEntity {
    ...
    public static List<TContact> fromHouse(HouseRent house){
...
//    	String publisherId = house.getBroker().getErpId();	//房源发布人ID
    	...
    	Set<HouseAppraise> appList = house.getAppraises();
    	for(HouseAppraise app: appList){
    		TContact c = new TContact();
    		if(app.getBroker()!=null && house.getBroker()!=null){
    			c.setFlag( app.getBroker().getErpId().equals(house.getBroker().getErpId())? "1" : "0");
    			//用于删除 
    			house.getBaiDuHouseRentList().setBrokerName(strToCDATA(app.getBroker().getBname(), "未知"));
    			house.getBaiDuHouseRentList().setTelephone(strToCDATA(app.getBroker().getTelephone(), "18967113307"));
    		}else
    			c.setFlag( "0");
    		c.setGender(TGender.男);  //TODO: 这里需要修改网站经纪人信息
    		if(app.getBroker()!=null){
	    		c.setName(strToCDATA(app.getBroker().getBname(), "未知"));
	    		c.setProfile( "http://" + StringUtil.HSHB_DOMAIN + "/frontImages" + app.getBroker().getPhotograph());
	    		c.setTelephone(strToCDATA(house.getBroker().getTelephone(), "18967113307"));
    		}else{
    			continue;
    		}
    		c.setType(T1Type.中介);
    		...
    		if(app.getBroker()!=null && house.getBroker()!=null)
	    		if(app.getBroker().getErpId().equals(house.getBroker().getErpId()))
    	...
    	if(!isFoundPublisher){
    		...
    		if(house.getBroker()!=null){
    			//用于删除
    			house.getBaiDuHouseRentList().setBrokerName(strToCDATA(house.getBroker().getBname(), "未知"));
    			house.getBaiDuHouseRentList().setTelephone(strToCDATA(house.getBroker().getTelephone(), "18967113307"));
	    		c.setName(strToCDATA(house.getBroker().getBname(), "未知"));
	    		c.setProfile("http://" + StringUtil.HSHB_DOMAIN + "/frontImages" + house.getBroker().getPhotograph());
	    		c.setTelephone(strToCDATA(house.getBroker().getTelephone(), "18967113307")); //todo:这里的号码是盛有富的小手机，以后可以换掉
    		}else{
    			System.out.println("No broker rentHouse: " + house.getHourseNo());
    			return null;
    		}
    		c.setType(T1Type.中介);
    		c.setWorksFor(StringUtil.HSHB_NAME);
